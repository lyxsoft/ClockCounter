...
* Function: void D1302ClockInit( void ) * 
...
* Function: unsigned char BCDHour24 (unsigned char, unsigned char)

* Description: Change to Hour24 format

* Parameter: Hour24 or Hour12 format
...
unsigned char BCDHour24 (unsigned char nBCD)
{
	if (nBCD & 0x80)	// 12 HOUR
	{
		...
		{
			nBCD = ((nBCD & 0x10) ? 10 : 0) + (nBCD & 0xF);
			if (nBCD != 12) //12 PM means 12 aclock
				nBCD += 12;
			return (((nBCD / 10) << 4) + (nBCD % 10));
		}
		else						//AM
		{
			nBCD &= 0x1F;
